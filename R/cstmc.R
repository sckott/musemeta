...
#' @param vocabulary_id (character) The id or name of a vocabulary, if give
#' only tags that belong to this vocabulary will be returned
#' @param all_fields (logical) Return full tag or group dictionaries instead of
#' just names. Default: `FALSE`
#' @param q Query terms, defaults to '*:*', or everything. It is a string of
#' the form `field:term` or a list of strings, each of the same form.  Within
#' each string, `field` is a field or extra field on the Resource domain
#' object. If `field` is hash, then an attempt is made to match the `term`
#' as a *prefix* of the Resource.hash field. If `field` is an extra field, then
...
#' @export
#' @rdname cstmc
cstmc_group_list <- function(offset=0, limit=31, sort = NULL, groups = NULL,
  all_fields = FALSE, as='list', ...) {
...
cstmc_group_show <- function(id, include_datasets = TRUE, as='list', ...){
  ckanr::group_show(id=id, include_datasets=include_datasets, url=cstmcbase(),
    as=as, ...)
}

#' @export
#' @rdname cstmc
cstmc_tag_list <- function(query = "*:*", vocabulary_id = NULL,
  all_fields = FALSE, as='list', ...) {
...
#' @export
#' @rdname cstmc
cstmc_package_search <- function(q = "*:*", fq = NULL, sort = NULL, rows = NULL,
...
cstmc_package_show <- function(id, use_default_schema = FALSE, as='list', ...) {
  ckanr::package_show(id = id, use_default_schema = use_default_schema,
    url=cstmcbase(), as=as, ...)
}

#' @export
#' @rdname cstmc
cstmc_resource_search <- function(q = "*:*", sort = NULL, offset = NULL,
  limit = NULL, as='list', ...) {
